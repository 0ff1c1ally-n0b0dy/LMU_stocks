...
file1_text = "./Prints/Output no events, change lr: epochs %d, hidden %d,memory %d,theta %d,window %d.txt" % (
...
for folder in folders_list:
    ...
    while epoch <= epochs and flag_error == 1:
        ...
        torch.save(model,
               "./Saved_weights/"+folder+" LMU trade, no events change lr: epochs %d, accuracy %.4f%%,hidden %d,memory %d,theta %d,batch %d,window %d.pth" %
    ...
    profit_data.to_csv(
        "./Profits/Profits LMU trade no events, change lr: epochs %d, accuracy/epoch %.4f%%,hidden %d,memory %d,theta %d,batch %d,window %d.csv" %
        (epoch, 100 * np.mean(accuracies_per_epoch), hidden_size, memory_size, theta, batch_size, window))

    if flag_error == 1:
        if length_data > window:
            scaled_data.to_csv("LMU no events, change lr predicted events %s.csv" % (ticker))

    file1.write("Sum of all profits after backtesting on %s and forwardtesting on %s is %f \n"% (",".join(folders_list),fwd_folder,sum(profits)))
    print("Sum of all profits after backtesting on %s and forwardtesting on %s is %f"% (",".join(folders_list),fwd_folder,sum(profits)))

    file1.write("Sum of all returns after backtesting on %s and forwardtesting on %s is %f \n" % (
    ",".join(folders_list), fwd_folder, sum(profits)/capital))
    print("Sum of all returns after backtesting on %s and forwardtesting on %s is %f" % (
    ",".join(folders_list), fwd_folder, sum(profits)/capital))
