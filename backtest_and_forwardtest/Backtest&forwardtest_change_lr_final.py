...
##########################
folders_list=["FolderID102a_D1_trades_handmarked for training"]
low_lr=low_lr_initial=3e-5
high_lr=high_lr_initial=10**(-4)
warmup_steps=4000
...
file1_text = "Output epochs %d, hidden %d,memory %d,theta %d,window %d.txt" % (
...
for folder in folders_list:
    ...
    while epoch <= epochs and flag == 1:
        ...
        for i in training_csv:
            ...
            n_train = len(data)
            total_steps+=epoch*n_train
            if n_train > 0 and flag_error == 1 and n_train - window > 0:
                step_num = total_steps
                low_lr = min(low_lr_initial,(input_size ** (-0.5) )* min(step_num**(-0.5),step_num*(warmup_steps**(-1.5))))
                ...
                events_column_index = columns.index("events." + ticker)
                n_var = len(columns)
                ...
                scaled_data["events." + ticker] = [0] * n_train
                for k in range(n_train - window):
                    ...
                    if x_train.size(0) != 0:
                        ...
                        if y_pred >= p_thr:
                            ...
                            output_class = 0
                        scaled_data["events." + ticker].iloc[k + window] = output_class
                ...
                file1.write("[%d/%d:%s/%s] Training completion:%.2f%%, Time elapsed:%s, Training loss: %.7f, lr=%.f \n"
                            % (epoch, epochs, folder, ticker, 100 * round(count_training / (nr_training_csv * epochs), 4),
                               timing(start), running_loss / (train_iters),low_lr))
                ...
                print("[%d/%d:%s/%s] Training completion:%.2f%%, Time elapsed:%s, Training loss: %.7f, lr=%.f"
                      % (
                      epoch, epochs, folder, ticker, 100 * round(count_training / (nr_training_csv * epochs), 4), timing(start),
        ...
        for i in training_csv:
            ...
            if n_train > 0 and flag_error == 1 and n_train - window > 0:
                ...
                high_lr = max(high_lr_initial,(input_size ** (-0.5)) * min((step_num ** (-0.5)), step_num * (warmup_steps ** (-1.5))))
                optimizer_high_lr = torch.optim.Adam(model.parameters(), lr=high_lr, amsgrad=True)
                file1.write("Training for %s in folder %s started with lr=%.f \n" % (
                ...
                for k in range(n_var):
                    if columns[k] != "events." + ticker:
                        scaler = MinMaxScaler()
                        scaled_data[columns[k]] = scaler.fit_transform(
                            np.array(scaled_data[columns[k]]).reshape(-1, 1))
                ...
                file1.write(
                        ...
                        % (epoch, epochs, folder, ticker, 100 * round(count_training / (nr_training_csv * epochs), 4),
                           timing(start), running_loss / (train_iters), high_lr))
                ...
                print("[%d/%d:%s/%s] Training completion:%.2f%%, Time elapsed:%s, Training loss: %.7f, lr=%.f"
                          % (
                              ...
                              100 * round(count_training / (nr_training_csv * epochs), 4), timing(start),
            ...
            for i in testing_csv:
                ...
                if n_test > 0 and flag_error == 1 and n_test - window > 0:
                    ...
                    file1.write(
                        ...
                        % (
                        epoch, epochs, folder, ticker, 100 * round(count_testing / (nr_testing_csv * epochs), 4), timing(start),
        ...
        torch.save(model,
               "./"+folder+" LMU trade, epochs %d, accuracy %.4f%%,hidden %d,memory %d,theta %d,batch %d,window %d.pth" %
                (epoch, 100 * np.mean(accuracies_per_epoch), hidden_size, memory_size, theta, batch_size,window))


    ###########################
    # Forward testing starts using the optimized weights from the backtesting folder
    # No need to load the weights now since they are already optimized in the code from above
    ###########################
    ################3
    ...
    for file_name in csvs:
        ...
        if flag_error == 1:
            if length_data > window:
                ...
                accuracies.append(accuracy / (j + 1))
                ...
                exit_index = [i + window for i in range(len_test - 1) if list_outputs_diff[i] == 1]

                capital = [[ @ CapitalAvailablePerTrade @]]
                    ...
                    while i <= len_entries_index - 1:
                        if entry_index[i] < window:
                            ...
                            if exit_index[i] - entry_index[i] >= 10 and data["price." + ticker].iloc[
                                ...
                                profit_end = data["price." + ticker].iloc[exit_index[i]]*data["size." + ticker].iloc[exit_index[i]]
                                ...
                                print("Return is %f" % (returns[-1]))

                            i += 1

        file1.write(
            "[%s] Testing completion %.2f%%, Time elapsed:%s, Testing loss: %.7f, Accuracy:%.4f%%, Overall Accuracy:%.4f%% \n"
            % (ticker, 100 * round(count_completion / nr_testing_csv, 4), timing(start), test_losses[-1],
    ...
    profit_data.to_csv(
        "./Profits/Profits LMU trade, events at once: epochs %d, accuracy %.4f%%,hidden %d,memory %d,theta %d,batch %d,window %d.csv" %
        (epoch, 100 * np.mean(accuracies_per_epoch), hidden_size, memory_size, theta, batch_size, window))

    if flag_error == 1:
        if length_data > window:
            scaled_data.to_csv("%s predicted events data.csv" % (ticker))

    file1.write("Sum of all profits after backtesting on %s and forwardtesting on %s is %.f \n"% (folder,fwd_folder,sum(profits))
    print("Sum of all profits after backtesting on %s and forwardtesting on %s is %.f"% (folder,fwd_folder,sum(profits))

    file1.write("Sum of all returns after backtesting on %s and forwardtesting on %s is %.f \n" % (
    folder, fwd_folder, sum(returns))
    print("Sum of all profits after backtesting on %s and forwardtesting on %s is %.f" % (
    folder, fwd_folder, sum(returns))
