...
file1_text = "./Prints/Output events at once: epochs %d, hidden %d,memory %d,theta %d,window %d, fixed low lr %f, fixed high lr %f.txt" % (
epochs, hidden_size, memory_size, theta, window, low_lr, high_lr)
    ...
    while epoch <= epochs and flag_error == 1:
        ...
        for i in training_csv:
            ...
            if n_train > 0 and flag_error == 1 and n_train - window > 0:
                ...
                n_iters=int((n_train - window)/n_at_once)
                for k in range(n_iters-1):
                    model.zero_grad()
                    X_train = scaled_data.values[n_at_once*k:(n_at_once*(k+1) + window), :]
                    for l in range(n_at_once):
                        x_train = X_train[l:(l+window), :]
                        x_train = torch.FloatTensor(x_train).to(device)
                        if x_train.size(0) != 0:
                            x_train = x_train.view(batch_size, window, n_var)
                            y_train = data["events." + ticker].iloc[n_at_once*k + window + l]
                ...
                if n_at_once*n_iters<n_train-window:
                    model.zero_grad()
                    X_train = scaled_data.values[n_at_once*n_iters:, :]
                    n_left_over = n_train-window-n_at_once*n_iters
                    for l in range(n_left_over):
                        x_train = X_train[l:(l+window), :]
                        x_train = torch.FloatTensor(x_train).to(device)
                        if x_train.size(0) != 0:
                            x_train = x_train.view(batch_size, window, n_var)
                            y_train = data["events." + ticker].iloc[n_at_once * n_iters + window + l]
                            y_train = torch.FloatTensor([y_train]).to(device)
                            y_pred = model(x_train)
                            if y_pred >= p_thr:
                                output_class = 1
        ...
        torch.save(model,
                   "./Saved_weights/" + folder + " LMU trade, b. events at once: epochs %d, accuracy %.4f%%,hidden %d,memory %d,theta %d,batch %d,window %d.pth" %
                   (epoch, 100 * np.mean(accuracies), hidden_size, memory_size, theta, batch_size, window))
