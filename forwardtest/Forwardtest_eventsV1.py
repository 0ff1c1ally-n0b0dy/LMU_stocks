...
file1_text = "./Prints/Output events at once: epochs %d, hidden %d,memory %d,theta %d,window %d.txt" % (
...
model=torch.load("./Saved_weights/Batch1 LMU trade, epochs 3, accuracy 72.6482%,hidden 32,memory 32,theta 512,batch 1,window 1000.pth",
                 map_location='cpu')
...
for file_name in csvs:
    ...
    if flag_error == 1:
        if length_data > window:
            ...
            n_iters = int((n_test - window) / n_at_once)
            for k in range(n_iters - 1):
                X_test = scaled_data.values[n_at_once * k:(n_at_once * (k + 1) + window), :]
                for l in range(n_at_once):
                    x_test = X_test[l:(l + window), :]
                    x_test = torch.FloatTensor(x_test).to(device)
                    if x_test.size(0) != 0:
                        x_test = x_test.view(batch_size, window, n_var)
                        y_test = data["events." + ticker].iloc[n_at_once * k + window + l]
                        ...
                        if y_pred >= p_thr:
                            ...
                            output_class = 0
                        scaled_data["events." + ticker].iloc[n_at_once * k + window + l] = output_class
            ...
            if n_at_once * n_iters < n_test - window:
                model.zero_grad()
                X_test = scaled_data.values[n_at_once * n_iters:, :]
                n_left_over = n_test - window - n_at_once * n_iters
                for l in range(n_left_over):
                    x_test = X_test[l:(l + window), :]
                    x_test = torch.FloatTensor(x_test).to(device)
                    if x_test.size(0) != 0:
                        x_test = x_test.view(batch_size, window, n_var)
                        y_test = data["events." + ticker].iloc[n_at_once * n_iters + window + l]
                        y_test = torch.FloatTensor([y_test]).to(device)
                        y_pred = model(x_test)
                        loss = criterion(y_pred, y_test)
                        test_loss += loss.item()
                        if y_pred >= p_thr:
                            ...
                            output_class = 0
                        scaled_data["events." + ticker].iloc[n_at_once * n_iters + window + l] = output_class
            ...
            accuracies.append(accuracy / test_iters)
            ...
            if len_entries_index > 0:
                ...
                while i <= len_entries_index - 1:
                    if entry_index[i] < window:
                        ...
                        # Initial entry inn the position
                        nr_shares = 0
                        profit_start = 0
                        ...
                        print("Entry index %d, exit index %d, price: %f and cutoff:%f" %
                              (entry_index[i], exit_index[i], data["price." + ticker].iloc[entry_index[i]],
                               indiff_price))
                        # If there are 10 consecutive ones.
                        if exit_index[i] - entry_index[i] >= 10 and data["price." + ticker].iloc[
                            ...
                            remaining_capital = capital
                            # If there are enough shares to buy in one trade the stock
                            if data["price." + ticker].iloc[entry_index[i]] * data["size." + ticker].iloc[
                                ...
                                remaining_capital -= data["price." + ticker].iloc[entry_index[i]] * \
                                                     data["size." + ticker].iloc[entry_index[i]]
                                j = entry_index[i] + 1
                                # as long as there is capital and the exiting hasn't started yet and the price is
                                # cheap enough according to the formula
...
profit_data.to_csv(
    "./Profits/Profits Fwd LMU trade, events at once:accuracy %.4f%%,hidden %d,memory %d,theta %d,batch %d,window %d.csv" %
    (100 * np.mean(accuracies), hidden_size, memory_size, theta, batch_size, window))

if flag_error == 1:
    if length_data > window:
        scaled_data.to_csv("LMU events at once, predicted events data for stock %s.csv" % (ticker))

file1.write("Sum of all profits for forwardtesting on %s is %.f \n" % (fwd_folder, sum(profits)))
print("Sum of all profits for forwardtesting on %s is %.f" % (fwd_folder, sum(profits)))

file1.write("Sum of all returns for forwardtesting on %s is %.f \n" % (fwd_folder, sum(profits) / capital))
print("Sum of all returns for forwardtesting on %s is %.f" % (fwd_folder, sum(profits) / capital))
